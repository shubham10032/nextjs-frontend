...
const LoanAgainstSecurity = ({ products }) => {
    return (
        <>
            {products.length > 0 && products.map((item, key) => (
                <LoanAgainstPropertyListLayout key={key} item={item} />
            ))}
        </>
    )
}

export default LoanAgainstSecurity

export const LoanAgainstSecurityListLayout = ({ item }) => {
    return <>
                    ...
                    <div className="col-lg-10 col-md-12">
                        <div className="xcard-charges x-loan">
                            ...
                            <div className="xcard-charge-join">
                                ...
                                <div className="trav">{JSON.parse(item.bankProductInfo)['net_salary_minimum'] != '' ? `${JSON.parse(item.bankProductInfo)['net_salary_minimum']} /m` : "N/A"} </div>
                            </div>
                            <div className="xcard-charge-join">
